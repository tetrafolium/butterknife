...
public void parentBindingInClasspath() throws IOException {
	JavaFileObject baseClassSource = JavaFileObjects.forSourceString(
		"test.Test",
		""
		+ "package test;\n"
	...
	JavaFileObject subClassSource = JavaFileObjects.forSourceString(
		"test.SubClass", ""
		+ "package test;\n"
		+ "import android.view.View;\n"
		+ "import butterknife.BindView;\n"
	...
	try (URLClassLoader compilationClasspath =
		     new URLClassLoader(new URL[] {classesOut.toURI().toURL()},
...
public void indirectViewRequiredInConstructor() throws IOException {
	JavaFileObject classpathClass = JavaFileObjects.forSourceString(
		"test.Test",
		""
		+ "package test;\n"
		+ "import android.view.View;\n"
	...
	JavaFileObject subclassInClasspath = JavaFileObjects.forSourceString(
		"test.SubClassTest", ""
		+ "package test;\n"
	...
	JavaFileObject toProcessSource = JavaFileObjects.forSourceString(
		"test.ToProcess", ""
		+ "package test;\n"
		+ "import android.view.View;\n"
		+ "import butterknife.BindView;\n"
...
private void compileSources(File classesOut, File sourcesOut,
                            JavaFileObject... sources) {
	JavaCompiler javaCompiler = ToolProvider.getSystemJavaCompiler();
	try {
		try (StandardJavaFileManager fileManager =
			     javaCompiler.getStandardFileManager(null, Locale.getDefault(),
			                                         UTF_8)) {
			JavaCompiler.CompilationTask javaCompilerTask = javaCompiler.getTask(
				null, fileManager, null,
				ImmutableList.of("-d", classesOut.getCanonicalPath(), "-s",
				                 sourcesOut.getCanonicalPath()),
				ImmutableList.of(), Arrays.asList(sources));
			javaCompilerTask.setProcessors(
				ImmutableList.of(new ButterKnifeProcessor()));
