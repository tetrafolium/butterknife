...
/**
 * Custom lint rule to make sure that generated R2 is not referenced outside
 * annotations.
 */
public class InvalidR2UsageDetector
    extends Detector implements Detector.UastScanner {
  private static final String LINT_ERROR_BODY =
      "R2 should only be used inside annotations";
  ...
  private static final Set<String> SUPPORTED_TYPES =
      ImmutableSet.of("array", "attr", "bool", "color", "dimen", "drawable",
                      "id", "integer", "string");

  static final Issue ISSUE = Issue.create(
      ISSUE_ID, LINT_ERROR_TITLE, LINT_ERROR_BODY, Category.CORRECTNESS, 6,
      Severity.ERROR,
      new Implementation(InvalidR2UsageDetector.class, Scope.JAVA_FILE_SCOPE));
  ...
  private static class R2UsageVisitor extends AbstractUastVisitor {
    ...
    R2UsageVisitor(JavaContext context) { this.context = context; }
    ...
    @Override
    public boolean
    visitQualifiedReferenceExpression(UQualifiedReferenceExpression node) {
    ...
    private static boolean endsWithAny(String text,
                                       Set<String> possibleValues) {
      String[] tokens = text.split("\\.");
      return tokens.length > 1 &&
          possibleValues.contains(tokens[tokens.length - 1]);
    }
  }
}
