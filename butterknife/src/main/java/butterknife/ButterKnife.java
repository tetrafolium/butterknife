 ...
 * finding views and attaching listeners by binding them with annotations. <p>
 ...
 * activity}, a
 * {@linkplain #bind(View) view}, or a {@linkplain #bind(Dialog) dialog}.
 * Alternate objects to bind can be specified along with an {@linkplain
 * #bind(Object, Activity) activity},
 ...
 * </code></pre>
 * <p>
 * Be default, views are required to be present in the layout for both field and
 * method bindings. If a view is optional add a {@code @Nullable} annotation for
 * fields (such as the one in the <a
 * href="http://tools.android.com/tech-docs/support-annotations">support-annotations</a>
 * library) or the {@code @Optional} annotation for methods. <pre><code>
 * {@literal @}Nullable @BindView(R.id.title) TextView subtitleView;
 * </code></pre>
 * Resources can also be bound to fields to simplify programmatically working
 * with views: <pre><code>
...
public final class ButterKnife {
  private ButterKnife() { throw new AssertionError("No instances."); }
  ...
  @CheckResult
  @UiThread
  private static Constructor<? extends Unbinder>
  findBindingConstructorForClass(Class<?> cls) {
    ...
    try {
      ...
      bindingCtor =
          (Constructor<? extends Unbinder>)bindingClass.getConstructor(
              cls, View.class);
      if (debug)
      ...
      throw new RuntimeException(
          "Unable to find binding constructor for " + clsName, e);
    }
